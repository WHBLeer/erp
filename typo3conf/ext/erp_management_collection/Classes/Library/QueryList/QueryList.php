...
namespace ERP\ErpManagementCollection\Lib;
...
class QueryList
{
     ...
     * @return mixed
     */
    public static function Query($page, $rules= [], $range = '', $outputEncoding = null, $inputEncoding = null,$removeHead = false)
    {
    ...
    {
        $args = func_get_args();
        count($args) || $args = array('ERP\ErpManagementCollection\Lib\QueryList');
        ...
        if (!empty($this->regRange)) {
            ...
            foreach ($robj as $item) {
                while (list($key, $reg_value) = each($this->regArr)) {
                    ...
                    switch ($reg_value[1]) {
                    case 'text':
                        $this->data[$i][$key] = mb_convert_encoding($this->_allowTags(pq($iobj)->html(),$tags), 'utf-8', 'gbk');
                        break;
                    case 'html':
