...
class FileGenerator
{
            ...
            foreach ($domainObject->getActions() as $action) {
                ...
                if ($action->getNeedsTemplate()
                    ...
                    if ($action->getNeedsForm()) {
                        ...
                        /** 
                         * 2020年2月28日 王宏彬修改,添加插件写入通知模板文件
                         */
                        if (!file_exists($partialDirectory . 'Messages.html')) {
                            $this->writeFile(
                                $partialDirectory . 'Messages.html',
                                $this->generateMessahesPartial($templateRootFolder . 'Partials/')
                            );
                        }
                    }
     ...
     * @throws \Exception
     */
    public function generateMessahesPartial($templateRootFolder)
    {
        return $this->renderTemplate($templateRootFolder . 'messages.htmlt', [
            'extension' => $this->extension
        ]);
    }
