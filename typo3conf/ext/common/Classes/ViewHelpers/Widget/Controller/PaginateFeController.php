...
namespace Sll\Common\ViewHelpers\Widget\Controller;
 ...
 *
 */
class PaginateFeController extends \TYPO3\CMS\Fluid\Core\Widget\AbstractWidgetController
{
    ...
    public function indexAction($currentPage = 1)
    {
        ...
        if ($this->currentPage > 1) {
            ...
            \Sll\Common\Utility\Page::setRegisterProperties(
    ...
    protected function buildPagination()
    {
        ...
        if ($this->currentPage===1) {
            $firstRecode = 1;
            if (count($this->objects) < $this->configuration['itemsPerPage']) {
                $lastRecode = count($this->objects);
            }else{
                $lastRecode = $this->configuration['itemsPerPage'];
            }
        } else{
            $firstRecode = ($this->configuration['itemsPerPage'] * ($this->currentPage-1)) + 1;
            if (count($this->objects) < ($this->configuration['itemsPerPage'] * $this->currentPage)) {
                $lastRecode = count($this->objects);
            }else{
                $lastRecode = ($this->configuration['itemsPerPage'] * $this->currentPage);
            }
        }
        ...
        $pagination = [
            ...
            'firstRecode' => $firstRecode,
            'lastRecode' => $lastRecode,
